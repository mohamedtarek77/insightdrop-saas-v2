...
function LoginContent() {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => setChecking(false), 3000); // safety fallback

    supabase.auth.getUser().then(({ data: { user } }) => {
      clearTimeout(timeout); // ← was missing before!
      if (user && !expired) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        // Full page navigation so middleware sees the new cookie immediately
